    ...
    class InstallAllFromInventory(WorkflowImpl):
        ...
        def workflow_builder(self, workflow_inputs: WorkflowInput) -> None:
            ...
            get_pages_cursors = SimpleTask(
                ...
                input_parameters=SimpleTaskInputParameters(root=dict(labels=get_labels.output_ref("labels_id"))),
            )
            ...
            loop_task = DoWhileTask(
                ...
                loop_over=[convert_to_string, get_pages_cursors_fork_task, fork, join],
            )
        ...
        def workflow_builder(self, workflow_inputs: WorkflowInput) -> None:
            self.tasks.append(
                SimpleTask(
                    ...
                    input_parameters=SimpleTaskInputParameters(root=dict(devices=workflow_inputs.devices.wf_input)),
                )
            )
    ...
    class UninstallAllFromInventory(WorkflowImpl):
        ...
        def workflow_builder(self, workflow_inputs: WorkflowInput) -> None:
            ...
            get_pages_cursors_fork_task = SimpleTask(
                ...
                input_parameters=SimpleTaskInputParameters(
                    root=dict(
                        task="INVENTORY_uninstall_in_batch", cursors_groups=convert_to_string.output_ref("result")
                    )
                ),
            )
