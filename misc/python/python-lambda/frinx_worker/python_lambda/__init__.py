...
class PythonLambda(WorkerImpl):
    ...
    def _wrap_lambda_func(worker_input: WorkerInput) -> str:
        return executed_func_template.format(custom_execution_commands=worker_input.lambda_function)
    ...
    def execute(self, worker_input: WorkerInput) -> TaskResult[Any]:
        ...
        return TaskResult(status=TaskResultStatus.COMPLETED, output=self.WorkerOutput(result=result))
