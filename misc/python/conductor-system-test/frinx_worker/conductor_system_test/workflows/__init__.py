        ...
        def workflow_builder(self, workflow_inputs: WorkflowInput) -> None:
            ...
            self.tasks.append(
                SimpleTask(
                    ...
                    input_parameters=SimpleTaskInputParameters(root=dict(time=workflow_inputs.sleep_time.wf_input)),
                )
            )
            ...
            echo_task = SimpleTask(
                ...
                input_parameters=SimpleTaskInputParameters(root=dict(input=generate_task.output_ref("text"))),
            )
            self.tasks.append(echo_task)

            self.output_parameters = self.WorkflowOutput(
                text=echo_task.output_ref("output"), bytes=generate_task.output_ref("bytes")
            )
